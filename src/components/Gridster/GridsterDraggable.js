...
export class GridsterDraggable {
  gridsterItem;// = new GridsterItemComponentInterface();// : GridsterItemComponentInterface;
  ...
  calculateItemPositionFromMousePosition(e = new MouseEvent()) {
  ...
  calculateItemPositionWithScale(e = new MouseEvent(), scale = 1) {
    if (this.gridster._options.dirType === DirTypes.RTL) {
  ...
  calculateItemPositionWithoutScale(e = new MouseEvent()) {
    if (this.gridster._options.dirType === DirTypes.RTL) {
  ...
  makeDrag() {
    if (this.gridster._options.draggable.dropOverItems && this.gridster.options.draggable &&
  ...
  calculateItemPosition() {
    ...
    if (this.positionXBackup !== this.gridsterItem.$item.x || this.positionYBackup !== this.gridsterItem.$item.y) {
      ...
      this.push.pushItems(direction, this.gridster._options.disablePushOnDrag)
      this.swap.swapItems()
      this.collision = this.gridster.checkCollision(this.gridsterItem.$item)
      if (this.collision) {
        this.gridsterItem.$item.x = this.positionXBackup
        this.gridsterItem.$item.y = this.positionYBackup
        if (this.gridster._options.draggable.dropOverItems && this.collision !== true && this.collision.$item) {
          this.gridster.movingItem = null
        }
      } else {
  ...
  dragStartDelay(e) {
    ...
    GridsterUtils.checkTouchEvent(e)
    if (!this.gridster._options.draggable.delayStart) {
      this.dragStart(e)
      return
    }
    const timeout = setTimeout(() => {
      this.dragStart(e)
      cancelDrag()
    }, this.gridster._options.draggable.delayStart)
